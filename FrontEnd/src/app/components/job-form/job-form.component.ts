...
export class JobFormComponent implements OnInit {
  ...
  loadJobData(id: number): void {
    ...
    this.jobService.getJobById(id).subscribe(
      ...
      (error) => {
        this.toastr.error(error.error?.message || 'Failed to load job data.', 'Error'); // ✅ Toastr error
        this.isLoading = false;
      }
    );
  }
  ...
  onSubmit(): void {
    if (this.jobForm.invalid) {
      this.jobForm.markAllAsTouched(); // Ensure that errors show up even if the user hasn't focused on the fields
      this.toastr.warning('Please fill out all required fields.', 'Validation'); // ✅ Toastr warning
      return;
    }
    ...
    if (this.isEditMode) {
      ...
      if (jobId) {
        this.jobService.updateJob(+jobId, jobData).subscribe(
          () => {
            this.toastr.success('Job updated successfully!', 'Success'); // ✅ Toastr success
            this.isLoading = false;
            setTimeout(() => {
          ...
          (error) => {
            this.toastr.error(error.error?.message || 'Failed to update the job.', 'Error'); // ✅ Toastr error
            this.isLoading = false;
          }
        );
      }
      ...
      this.jobService.createJob(jobData).subscribe(
        () => {
          this.toastr.success('Job posted successfully!', 'Success'); // ✅ Toastr success
          this.isLoading = false;
          this.jobForm.reset();
        ...
        (error) => {
          this.toastr.error(error.error?.message || 'Failed to post job.', 'Error'); // ✅ Toastr error
          this.isLoading = false;
        }
      );
    }
